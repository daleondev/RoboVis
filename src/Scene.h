...
{
public:
    typedef std::unordered_map<std::string, std::shared_ptr<Entity>>::iterator EntityIterator;

    static void createMesh(const std::string& name, const aiScene* meshSource, const glm::mat4& initialTransformation = glm::mat4(1.0f));
    static void createMarker(const std::string& name, const glm::mat4& initialTransformation = glm::mat4(1.0f));
    ...
    inline static EntityIterator entityBegin() { return s_entities.begin(); }
    inline static EntityIterator entityEnd() { return s_entities.end(); }
